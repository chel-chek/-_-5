...
BUILDING_INCOME = {
    'business_center': 25, 'lumberjack': 80, 'construction_factory': 100,
    'university': 2, 'fabric_factory': 50, 'stables': 20, 'barracks': 0,
    'oil_rig': 150, 'iron_mine': 150, 'coal_mine': 125,
}
...
def can_collect_income(uid):
    """Проверка сбора дохода с накоплением"""
    p = get_player(uid)
    if not p: return True, 1, None
    last = p[15]  # last_collection
    if not last or str(last) in ['0', '0.0', 'None', 'NULL', '']:
        return True, 1, None
    try:
        last_dt = datetime.strptime(str(last)[:19], "%Y-%m-%d %H:%M:%S")
        passed = (datetime.now() - last_dt).total_seconds()
        cycles = int(passed // 86400)
        if cycles >= 1:
...
def get_buildings_income(uid):
    ...
    if 'business_center' in bld: total_tenge += 25 * bld['business_center']
    if 'lumberjack' in bld: total_wood += 80 * bld['lumberjack']
    if 'construction_factory' in bld: total_cement += 100 * bld['construction_factory']
    if 'university' in bld: total_science += 2 * bld['university']
    if 'iron_mine' in bld: total_iron += 150 * bld['iron_mine']
    if 'oil_rig' in bld: total_fuel += 150 * bld['oil_rig']
    if 'coal_mine' in bld: total_coal += 125 * bld['coal_mine']
    if 'fabric_factory' in bld: total_fabric += 50 * bld['fabric_factory']
    if 'stables' in bld: total_horses += 20 * bld['stables']
    
    return {
        'tenge': total_tenge, 'wood': total_wood, 'cement': total_cement,
        'science_points': total_science, 'iron': total_iron, 'fuel': total_fuel,
        'coal': total_coal, 'fabric': total_fabric, 'horses': total_horses
    }
...
def cmd_collect(message):
    ...
    # Начисляем ресурсы с учётом накопления
...
def cmd_expedition(message):
    ...
    if not can:
        bot.reply_to(message, f"❌ Экспедиция недоступна! Следующая через {remaining}")
        return
...
def cmd_craft(message):
    ...
    try:
        ...
        if not c.fetchone():
            c.execute("INSERT INTO blueprints VALUES (?,?)", (name, uid))
        db_conn.commit()
